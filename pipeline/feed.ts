...
/** "-30" -> "-30%", "0" -> "back at the peak". */
const drawdownWording = (state: string): string =>
  state === '0' ? 'back at the running peak' : `past ${state}% from the running peak`;
...
export function signalFeed(
  ...
  for (let i = 1; i < dd.length; i++) {
    ...
    entries.push({
      ...
      date: to.since,
      title: deepened ? `Drawdown deepened ${drawdownWording(to.state)}` : `Drawdown recovered to ${to.state === '0' ? 'the peak' : `${to.state}%`}`,
      summary:
